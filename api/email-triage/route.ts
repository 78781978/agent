...
function localTriage(emails: string[]) {
  const blocks = emails.map((email, index) => {
    ...
    return [
      ...
      `| Kategoria | ${category} |`,
      `| Priorytet | ${priority} |`,
      `| Uzasadnienie | ${isSpam ? "Wiadomość wygląda na masową próbę wyłudzenia." : isComplaint ? "Wymaga szybkiej reakcji, bo dotyczy problemu klienta lub terminu." : isMeeting ? "Może prowadzić do współpracy, ale nie wymaga reakcji natychmiast." : "Wiadomość informacyjna, można obsłużyć później."} |`,
      "",
      "**Proponowana odpowiedź:**",
      `> ${draft}`,
      "",
